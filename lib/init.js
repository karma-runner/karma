...
// TODO: update tests to not rely on internal module state and change this to arrow function
function installPackage (pkgName) {
  ...
  exec('npm install ' + pkgName + ' --save-dev', options, (err, stdout, stderr) => {
...
const validateBrowser = (name) => {
  // TODO(vojta): check if the path resolves to a binary
  installPackage('karma-' + name.toLowerCase().replace('canary', '') + '-launcher')
}

const validateFramework = (name) => {
  installPackage('karma-' + name)
}

const validateRequireJs = (useRequire) => {
  if (useRequire) {
    validateFramework('requirejs')
  }
}

// TODO: update tests to not rely on internal module state and change this to const
var questions = [{
  id: 'framework',
...
exports.init = (config) => {
  ...
  let colorScheme = COLOR_SCHEME.ON
  ...
  process.stdin.on('keypress', (s, key) => sm.onKeypress(key))
