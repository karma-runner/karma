 ...
 */
export class CaptureTimeoutLauncher {
  state
  STATE_BEING_CAPTURED
  name
  error
  kill
  on

  constructor(timer, captureTimeout) {
    if (!captureTimeout) {
      return
    }
  ...
  static decoratorFactory(timer, /* config.captureTimeout */ captureTimeout) {
