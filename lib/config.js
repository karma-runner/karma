...
var normalizeConfig = function(config, configFilePath) {
  ...
  if (config.protocol !== constant.HTTP_PROTOCOL && config.protocol !== constant.HTTPS_PROTOCOL) {
    log.warn('Invalid configuration: protocol must be %s or %s, set to %s', constant.HTTP_PROTOCOL,
        constant.HTTPS_PROTOCOL, constant.DEFAULT_PROTOCOL);
    config.protocol = constant.DEFAULT_PROTOCOL;
  }

  config.oneTimeCert = (config.protocol === constant.HTTPS_PROTOCOL && (!config.cert.key || !config.cert.cert));
