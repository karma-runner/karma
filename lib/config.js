...
var normalizeConfig = function (config, configFilePath) {
  ...
  Object.keys(preprocessors).forEach(function (pattern) {
    ...
    // Handle minimatch inverse patterns
    if (pattern[0] === '!') {
      // Slice off the first character and prepend it
      normalizedPattern = '!' + helper.normalizeWinPath(basePathResolve(pattern.slice(1)))
    } else {
      normalizedPattern = helper.normalizeWinPath(basePathResolve(pattern))
    }
