...
var List = function(patterns, excludes, emitter, preprocess, batchInterval) {
  ...
  var resolveFiles = function(buckets) {
    ...
    var files = {
      served: [],
      included: [], // included through a script tag
      jsIncluded: {} // inclusion through javascript (managed by adapters)
    };

    buckets.forEach(function(bucket, idx) {

      var included = patterns[idx].included;
      if (included && included !== true) {
        if (!files.jsIncluded[included]) {
          files.jsIncluded[included] = [];
        }
      }
        ...
        if (!uniqueMap[file.path]) {
          ...
          if (included) {
            if (included === true) {
              ...
              files.jsIncluded[included].push(file);
            }
          }
