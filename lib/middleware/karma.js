...
// NB. This only works correctly if directories have a trailing slash
var relativePath = function (requestUrl, urlPath) {
  var requestParts = requestUrl.split('/')
  var pathParts = urlPath.split('/')
  while (requestParts[0] === pathParts[0]) {
    requestParts.shift()
    pathParts.shift()
  }
  var prefix = ''
  // -1 here because files are relative to other files and we will have an extra component
  // if the requestUrl is a directory due to the requirement to have a trailing slash
  var up = requestParts.length - 1
  while (up > 0) {
    prefix += '../'
    up--
  }
...
var filePathToUrlPath = function (filePath, basePath, requestUrl) {
  if (filePath.indexOf(basePath) === 0) {
    return relativePath(requestUrl, '/base' + filePath.substr(basePath.length))
  }
  return relativePath(requestUrl, '/absolute' + filePath)
}
  ...
  return function (request, response, next) {
    ...
    if (isRequestingJsFile) {
      return serveStaticFile(requestUrl, response, function (data) {
        return data.replace('%KARMA_SOCKET_RELATIVE_URL%', relativePath(requestUrl, '/socket.io'))
      ...
      return filesPromise.then(function (files) {
        ...
        fileServer(requestedFileUrl, response, function (data) {
          ...
          var scriptTags = files.included.map(function (file) {
            ...
            if (!file.isUrl) {
              filePath = filePathToUrlPath(filePath, basePath, requestUrl)
          ...
          var mappings = files.served.map(function (file) {
            ...
            var filePath = filePathToUrlPath(file.path, basePath, requestUrl).replace(/\\/g, '\\\\')
      ...
      return filesPromise.then(function (files) {
        ...
        response.end(JSON.stringify({
          files: files.included.map(function (file) {
            return filePathToUrlPath(file.path + '?' + file.sha, basePath, requestUrl)
          })
        }))
      })
    }
