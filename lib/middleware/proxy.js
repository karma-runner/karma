...
var createProxyHandler = function(proxy, proxyConfig, proxyValidateSSL, urlRoot) {
  ...
  return {
    ...
    upgrade: function(request, socket, head) {
      if (request.url.indexOf(urlRoot) === 0) return;
      for (var i = 0; i < proxiesList.length; i++) {
        if (request.url.indexOf(proxiesList[i]) === 0) {
          var proxiedUrl = proxies[proxiesList[i]];
          log.debug('upgrade proxyWebSocketRequest %s to %s:%s', request.url, proxiedUrl.host, proxiedUrl.port);
          proxy.proxyWebSocketRequest(request, socket, head, { host: proxiedUrl.host, port: proxiedUrl.port });
        }
      }
    }
  };
...
exports.create = function(/* config */ config, /* config.proxies */ proxies, /* config.proxyValidateSSL */ validateSSL) {
  log.debug('createProxyHandler urlRoot=%s', config.urlRoot);
  proxyHandler = createProxyHandler(new httpProxy.RoutingProxy({changeOrigin: true}), proxies, validateSSL, config.urlRoot);
