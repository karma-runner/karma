...
var parseProxyConfig = function (proxies, config) {
  ...
  return _.sortBy(_.map(proxies, function (proxyConfiguration, proxyPath) {
    ...
    var useEnvironmentProxy = 'useEnvironmentProxy' in proxyConfiguration ? proxyConfiguration.useEnvironmentProxy : true
    var agent
    if (useEnvironmentProxy) {
      var upstreamProxy = process.env[https ? 'https_proxy' : 'http_proxy'] || process.env[https ? 'HTTPS_PROXY' : 'HTTP_PROXY']
      if (upstreamProxy) {
        log.info('using proxy from environment "%s"', upstreamProxy)
        agent = new HttpsProxyAgent(upstreamProxy)
      }
    }
