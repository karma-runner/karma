...
exports.start = function(cliOptions, done) {
  ...
  webServer.listen(config.port, function() {
    log.info('Testacular server started at http://' + config.host + ':' + config.port + config.urlRoot);

    if (config.browsers && config.browsers.length) {
      launcher.launch(config.browsers, config.host, config.port, config.urlRoot, config.captureTimeout, 3);
    }
  });
  ...
  var tryExecution = function() {
    ...
    if (!capturedBrowsers.length) {
      log.warn('No captured browser, open http://' + config.host + ':' + config.port + config.urlRoot);
      return false;
  ...
  var runnerServer = net.createServer(function (socket) {
    ...
    if (!capturedBrowsers.length) {
      log.warn('No captured browser, open http://' + config.host + ':' + config.port + config.urlRoot);
      socket.end('No captured browser, open http://' + config.host + ':' + config.port + config.urlRoot + '\n');
      return;
    }
