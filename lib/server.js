...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  webServer.listen(config.port, function() {
    log.info('Karma v%s server started at %s://%s:%s%s', constant.VERSION, config.protocol,
        config.hostname, config.port, config.urlRoot);
...
exports.start = function(cliOptions, done) {
  ...
  // if we need to create a one-time self-signed cert, do so asynchronously and then
  // invoke injector.start
  if (config.protocol === constant.HTTPS_PROTOCOL && (!config.cert.key || !config.cert.cert)) {
    require('pem').createCertificate({days:1, selfSigned:true}, function(err, keys) {
        config.cert.onetime = {key: keys.serviceKey, cert: keys.certificate};
