...
exports.start = function(cliOptions) {
  ...
  if(config.customReporter){
      var customReporter = config.customReporter;
      var customResultReporter = reporter.createCustomReporter(customReporter.path, customReporter.options || {}, config.basePath, config.urlRoot);
      globalEmitter.bind(customResultReporter);
  }
