    ...
    if (requestUrl === '/context.html' || requestUrl === '/debug.html') {
      return promiseContainer.promise.then(function(files) {
        serveStaticFile(staticFolder + requestUrl, response, function(data, response) {
          ...
          var jsIncludes = {};
          Object.keys(files.jsIncluded || {}).forEach(function(includeType) {
            jsIncludes[includeType] = files.jsIncluded[includeType].map(function(file) {
              return getFilePath(file);
            });
          });

          jsIncludes = 'window.__testacular__.jsIncludes = ' + JSON.stringify(jsIncludes) + ';\n';

          return data.replace('%SCRIPTS%', scriptTags.join('\n')).replace('%MAPPINGS%', mappings).replace('%JSINCLUDES%', jsIncludes);
        });
      });
    }
