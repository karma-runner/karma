...
var TeamcityReporter = function(formatError, reportSlow) {
  ...
  this.onRunComplete = function(browsers, results) {
    ...
    Object.keys(this.browserResults).forEach(function(key){
      var browserResult = that.browserResults[key]; 
      if(browserResult.suits.length > 0){
        browserResult.log.push(util.format(that.SUITE_END, escTCString(browserResult.suits[browserResult.suits.length - 1])));
      }
      that.write(that.BROWSER_START, key);
      that.write(browserResult.log.join("\n"));
