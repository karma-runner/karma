...
exports.resolve = function (plugins, emitter) {
  ...
  plugins.forEach(function (plugin) {
    if (helper.isString(plugin)) {
      ...
      var pluginDirectory = path.normalize(path.join(process.cwd(), 'node_modules'))
      var regexp = new RegExp('^' + plugin.replace('*', '.*'))

      // some ci test will fail while I just load the cwd + node_modules
      // try to load these two types to fix this
      var pluginNames = _.union(
          fs.readdirSync(path.normalize(path.join(__dirname, '/../..'))),
          fs.readdirSync(path.normalize(path.join(process.cwd(), 'node_modules')))
      )

      log.debug('Loading %s from %s', plugin, pluginDirectory)
      pluginNames.filter(function (pluginName) {
